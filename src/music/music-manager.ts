...
import { SafeYtDlpPlugin } from './safe-yt-dlp-plugin';
import { SafeSpotifyPlugin, SafeSpotifyPluginOptions } from './safe-spotify-plugin';
...
export class MusicManager {
  ...
  public stopAndClearQueue(guildId: string) {
    const queue = this.distube.getQueue(guildId);
    if (!queue) return;

    this.disableRadioMode(guildId);
    queue.stop();
    this.activityManager.onFinish(queue);
